...
**警惕现象：** 中文正文里混用英文半角标点，例如 `,`、`.`、`:`、`;`、`?`、`!`、`()`、`"..."`；中文引号写成英文直引号；中文句子里把逗号写成英文 comma；省略号写成 `...`；破折号写成 `--`。

**规范：** 参考 W3C《中文排版需求》（clreq）的中文排版原则，中文横排正文优先使用中文全角标点：`，。；：？！、（）《》“”`。简体中文横排通常使用弯引号 `“”`；竖排场景可按排版规范使用直角引号。英文、代码、命令、URL、文件名、API 名称、版本号、数学表达式和 Markdown 语法内部保留原标点。

**问题：** AI 改写中文时常保留英文标点，或把中英文标点混在一起。读起来像机器拼接，也破坏中文排版一致性。

**改写前：**
> 这个功能支持"自动分类", "工单分派"和"进度追踪". 如果用户需要帮助, 可以点击"Help"按钮.

**改写后：**
> 这个功能支持“自动分类”“工单分派”和“进度追踪”。如果用户需要帮助，可以点击 “Help” 按钮。

**注意：** 不要机械替换所有半角符号。`README.md`、`GET /api/users`、`v1.2.0`、`foo(bar)`、`https://example.com?a=1`、`"strict"`、`[链接](https://example.com)`、`| 表格 |`、`## 标题` 这类代码、命令、URL、Markdown 语法或英文片段应保持原样，必要时在中英文之间留空格以提升可读性。
...
**警惕结构：** 不是 A，而是 B；不是 A，不是 B，而是 C；不是 A，不是 B，是 C；真正重要的不是……而是……；真正重要的不是……是……；问题不在于……而在于……；核心不是……而是……；我们缺的不是……而是……。

**问题：** 这种句式本身可以用，但 AI 经常反复使用它来制造“洞察感”“纠偏感”或“金句感”。如果一段文字里连续出现，读者会感觉作者总在摆出“我要告诉你真正答案”的姿态，而不是直接说明问题。

**改写原则：**

- 如果 A 只是铺垫，直接删掉 A，写 B。
- 如果 A 和 B 都重要，改成并列事实，不要强行否定其中一边。
- 如果想表达取舍，说明具体差异：谁受影响、哪个环节出问题、为什么 B 更关键。
- 一篇文章中不要频繁使用同类否定转折句式；保留最有必要的一处即可。
